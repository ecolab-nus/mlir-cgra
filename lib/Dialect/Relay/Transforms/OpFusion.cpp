...
#include "soda/Dialect/Relay/Passes.hpp"
#include "soda/Misc/Common.hpp"
...
LogicalResult OpFusionPattern::matchAndRewrite(
    ...
    auto funcName = llvm::formatv("fused_{}", nGroup++);
    rewriter.setInsertionPointToEnd(root->getParentOp()->getBlock());
    auto func =
        func::FuncOp::create(root->getLoc(), funcName.str(), funcType, {});
